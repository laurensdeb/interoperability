 ...
 *
 * Adds a method to receive settings for WebIds known to the RS.
 */
export class ExtendedAccountStore implements AccountStore {
  ...
  /**
   * @param {KeyValueStorage<string, EmailPasswordData>} storage
   * @param {ExpiringStorage<string, EmailPasswordData>} forgotPasswordStorage
   * @param {number} saltRounds
   * @param {number} forgotPasswordExpiration
   */
  ...
  public async getWebIdSettings(): Promise<Map<string, AccountSettings>> {
    const entries = this.storage.entries();
    const result = new Map<string, AccountSettings>();

    for await (const i of entries) {
      // TODO: this is ugly
      if (i[0].startsWith('http')) {
        result.set(i[0], i[1] as AccountSettings);
      }
    }
